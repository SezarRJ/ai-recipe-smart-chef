  ...
  return (
    <PageContainer header={{ title: t('Scan Dish', 'مسح الطبق'), showBackButton: true }}>
      <div className="space-y-6 pb-6">
        <ScanDishComponent onScanComplete={handleScanResult} />
        
        {scanResult && (
          <Card>
            <CardContent className="p-4 space-y-4">
              <div className="text-center">
                <h2 className="text-xl font-bold text-wasfah-deep-teal">{scanResult.dishName}</h2>
              </div>
              
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-wasfah-light-gray/50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">{t('Calories', 'سعرات حرارية')}</p>
                  <p className="font-bold">{scanResult.nutrition.calories} kcal</p>
                </div>
                <div className="bg-wasfah-light-gray/50 p-3 rounded-lg">
                  <p className="text-xs text-gray-500">{t('Protein', 'بروتين')}</p>
                  <p className="font-bold">{scanResult.nutrition.protein}g</p>
                </div>
                <div className="bg-wasfah-light-gray/50 p-3 rounded-lg">
