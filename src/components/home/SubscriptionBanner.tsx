  ...
  return (
    <Link to="/subscription">
      <Card className="relative overflow-hidden border-2 border-gradient-to-r from-yellow-400 to-orange-500 bg-gradient-to-r from-yellow-50 via-orange-50 to-red-50 hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02]">
        ...
        <div className="absolute bottom-0 left-0 w-12 h-12 bg-orange-400/20 rounded-full blur-lg"></div>
        
        <div className={`p-4 relative ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
          <div className={`flex items-center justify-between ${direction === 'rtl' ? 'flex-row-reverse' : ''}`}>
            <div className={`flex-1 ${direction === 'rtl' ? 'text-right' : 'text-left'}`}>
              ...
              <div className={`flex items-center space-x-3 text-xs text-gray-500 ${direction === 'rtl' ? 'space-x-reverse' : ''}`}>
            ...
            <div className={`flex items-center ${direction === 'rtl' ? 'flex-row-reverse' : ''}`}>
              <Button
                size="sm"
                className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-semibold shadow-md text-xs px-3 py-1"
              >
                {t("Upgrade Now", "ترقية الآن")}
              </Button>
              <ChevronRight className={`h-4 w-4 text-orange-500 ml-2 transition-transform duration-300 group-hover:translate-x-1 ${direction === 'rtl' ? 'rotate-180 mr-2 ml-0' : ''}`} />
            </div>
          </div>
        </div>
      </Card>
